...
export default function Perfil() {
  useAuthGuard();
  const { data: perfil, erro, carregando, recarregar } = useApiData(() => getProfessorPerfil(), []);
  ...
  const salvar = async (e) => {
    ...
    setErroSalvar("");
    try {
      await updateProfessorPerfil(form);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-400">Atualize seus dados de docente.</p>
      </div>
      ...
      {carregando ? (
        ...
        <form onSubmit={salvar} className="space-y-5 rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
          ...
          <div className="flex justify-end pt-2">
            <button type="submit" disabled={salvando} className="rounded-xl bg-purple-600 px-8 py-3 text-sm font-semibold text-white shadow-sm hover:bg-purple-700 disabled:opacity-50">
