...
import { addLinkAoProjeto } from "../../../lib/projetosApi";
...
export default function Submeter() {
  ...
  const handleAddLink = () => {
    setFormData({
      ...formData,
      links: [...formData.links, { rotulo: "", url: "" }],
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.professor_orientador_id) {
      alert("Selecione um professor orientador.");
      return;
    }

    const linksValidos = formData.links.filter(
      (link) => link.url.trim() !== "",
    );

    const linkIncompleto = linksValidos.some(
      (link) => !/^https?:\/\//i.test(link.url.trim()),
    );

    if (linkIncompleto) {
      alert("Os links devem começar com http:// ou https://.");
      return;
    }
    ...
    try {
      ...
      const projetoCriado = await submitProjeto(payload);

      await Promise.all(
        linksValidos.map((link) =>
          addLinkAoProjeto(projetoCriado.id, {
            url: link.url.trim(),
            descricao: link.rotulo.trim() || null,
          }),
        ),
      );

      alert("Projeto salvo como rascunho com sucesso!");

      navigate("/aluno/projetos", {
        replace: true,
      });
    } catch (err) {
      alert(`Erro ao salvar o projeto: ${err.message || "Tente novamente."}`);
    }
  };
    ...
    <div className="animate-in fade-in zoom-in-95 duration-200 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Cadastre as informações do projeto e salve-o como rascunho.
        </p>
      </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    professor_orientador_id: e.target.value,
                  })
                }
              >
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-sm font-semibold text-[#f19f17] hover:text-[#d68a12] transition-colors"
                >
                  + Adicionar link
                </button>
              </div>
                ...
                {formData.links.map((link, idx) => (
                  <div key={idx} className="flex gap-3">
                    <input
                      ...
                      value={link.rotulo}
                      onChange={(e) =>
                        handleLinkChange(idx, "rotulo", e.target.value)
                      }
                    />
                    <input
                      type="url"
              ...
              <p className="text-xs text-gray-400 mt-2">
                Adicione links relevantes como repositório, protótipo, vídeo de
                demonstração, etc.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
          ...
          <div className="border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center hover:bg-gray-50 transition-colors cursor-pointer group">
            ...
            <p className="text-sm text-gray-600 mb-1">
              ...
              <span className="text-[#f19f17] font-semibold">
                clique para selecionar
              </span>
            </p>
            <p className="text-xs text-gray-400">
              PDF, DOCX, PPTX - Máximo 50MB por arquivo
            </p>
          </div>
        </div>
