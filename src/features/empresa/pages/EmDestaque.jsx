...
export default function EmDestaque() {
  ...
  const outrosMocks = [
    { titulo: "App de Mobilidade Urbana Inteligente", curso: "Sistemas de Informação - SI-2A", desc: "Aplicativo que reduz 25% do tempo de deslocamento com rotas otimizadas por inteligência artificial.", media: "82%", aval: 3, badge: "Bom" },
    { titulo: "Plataforma de Ensino Adaptativo com IA", curso: "Ciência da Computação - CC-3B", desc: "Sistema que adapta o conteúdo educacional ao ritmo de aprendizagem individual de cada aluno.", media: "80%", aval: 2, badge: "Bom" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12 relative">
        ...
        <div className="lg:order-1 bg-white rounded-3xl border-2 border-[#f19f17] p-8 shadow-md transform lg:-translate-y-4 relative flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-lg font-bold text-gray-900 leading-snug mb-2">{top3Mocks[0].titulo}</h3>
            <p className="text-xs text-gray-400 mb-4">{top3Mocks[0].curso}</p>
            <p className="text-sm text-gray-600 mb-6 leading-relaxed line-clamp-3">{top3Mocks[0].desc}</p>
            <div className="flex flex-wrap gap-2 mb-8">
              {top3Mocks[0].tags.map(t => <span key={t} className="px-3 py-1 bg-gray-50 text-gray-500 rounded-lg text-[10px] font-semibold border border-gray-100">{t}</span>)}
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-gray-100 pt-4">
             ...
             <span className="px-3 py-1 rounded bg-emerald-50 text-emerald-600 text-xs font-bold">{top3Mocks[0].aval}</span>
          </div>
        </div>
        ...
        <div className="lg:order-2 bg-white rounded-3xl border border-gray-200 p-8 shadow-sm relative flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-lg font-bold text-gray-900 leading-snug mb-2">{top3Mocks[1].titulo}</h3>
            <p className="text-xs text-gray-400 mb-4">{top3Mocks[1].curso}</p>
            <p className="text-sm text-gray-600 mb-6 leading-relaxed line-clamp-3">{top3Mocks[1].desc}</p>
            <div className="flex flex-wrap gap-2 mb-8">
              {top3Mocks[1].tags.map(t => <span key={t} className="px-3 py-1 bg-gray-50 text-gray-500 rounded-lg text-[10px] font-semibold border border-gray-100">{t}</span>)}
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-gray-100 pt-4">
             <div className="flex items-center gap-1 font-bold text-[#f19f17]">
             ...
             <span className="px-3 py-1 rounded bg-emerald-50 text-emerald-600 text-xs font-bold">{top3Mocks[1].aval}</span>
          </div>
        </div>
        ...
        <div className="lg:order-3 bg-white rounded-3xl border border-amber-200 p-8 shadow-sm relative flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-lg font-bold text-gray-900 leading-snug mb-2">{top3Mocks[2].titulo}</h3>
            <p className="text-xs text-gray-400 mb-4">{top3Mocks[2].curso}</p>
            <p className="text-sm text-gray-600 mb-6 leading-relaxed line-clamp-3">{top3Mocks[2].desc}</p>
            <div className="flex flex-wrap gap-2 mb-8">
              {top3Mocks[2].tags.map(t => <span key={t} className="px-3 py-1 bg-gray-50 text-gray-500 rounded-lg text-[10px] font-semibold border border-gray-100">{t}</span>)}
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-gray-100 pt-4">
             <div className="flex items-center gap-1 font-bold text-[#f19f17]">
             ...
             <span className="px-3 py-1 rounded bg-blue-50 text-blue-600 text-xs font-bold">{top3Mocks[2].aval}</span>
          </div>
        </div>
      </div>
            ...
            <article key={i} className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl hover:border-gray-300 transition-colors gap-6">
              ...
              <div className="flex items-center gap-6 shrink-0 md:ml-auto w-full md:w-auto justify-end border-t border-gray-100 pt-4 md:pt-0 md:border-none">
                 <div className="text-center">
                   <span className="block font-bold text-gray-900">{o.media}</span>
                   <span className="text-[10px] text-gray-400 uppercase tracking-wider">Média</span>
                 </div>
                 <div className="text-center">
                   <span className="block font-bold text-gray-900">{o.aval}</span>
                   <span className="text-[10px] text-gray-400 uppercase tracking-wider">Aval.</span>
                 </div>
                 <div className="w-16 text-right">
                   <span className="px-2 py-1 rounded bg-amber-50 text-amber-600 text-xs font-bold">{o.badge}</span>
                 </div>
              </div>
            </article>
          ))}
